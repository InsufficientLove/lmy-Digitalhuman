...
echo ================================================================
echo 🎵 MuseTalk数字人系统安装脚本 - 增强进度版
echo ================================================================
echo 策略: 进度显示 + 强制版本锁定 + 磁盘检查 + 依赖清理
echo 解决: 版本冲突 + 空间不足 + 用户体验 + 实时反馈
echo.
...
REM 检查磁盘空间
echo 📊 检查磁盘空间...
for /f "tokens=3" %%a in ('dir F:\ ^| findstr "bytes free"') do (
    if %%a lss 5000000000 (
        echo ❌ F盘可用空间不足5GB，当前可用: %%a bytes
        echo 请清理磁盘空间或更换安装位置
        goto :final_pause
    )
)
...
pip install "numpy==1.24.4" --force-reinstall --no-deps -i https://pypi.tuna.tsinghua.edu.cn/simple/
...
pip install "protobuf>=3.11,<4.0" -i https://pypi.tuna.tsinghua.edu.cn/simple/
echo ✅ protobuf安装完成
...
pip install "opencv-python==4.8.1.78" --force-reinstall -i https://pypi.tuna.tsinghua.edu.cn/simple/
echo ✅ OpenCV安装完成
...
pip install "mediapipe==0.10.9" --force-reinstall -i https://pypi.tuna.tsinghua.edu.cn/simple/
echo ✅ MediaPipe安装完成
...
echo """MuseTalk数字人服务 - 增强版""" >> "%BASE_DIR%\musetalk_service.py"
...
echo """MuseTalk环境测试 - 增强版""" >> "%BASE_DIR%\test_environment.py"
...
echo print("MuseTalk数字人系统环境测试 - 增强版") >> "%BASE_DIR%\test_environment.py"
