...
echo ================================================================
echo 🚀 数字人系统完整安装脚本 - 针对您环境的增强版
echo ================================================================
echo 支持D盘VS安装和VS内置CMake检测
echo 专门优化您的F盘Python + D盘VS2022环境
echo.
echo 版本: 2025-01-28 增强版
...
if "%VS_PATH%"=="" (
    ...
    echo ✅ 使用Visual Studio %VS_YEAR%: %VS_PATH%
)

REM 增强的CMake检测 - 优先VS内置
echo 🔍 检测CMake（优先VS内置）...
set "CMAKE_PATH="
set "CMAKE_TYPE="
...
if "%CMAKE_PATH%"=="" (
    ...
    echo ✅ 使用CMake (%CMAKE_TYPE%): %CMAKE_PATH%
)
...
REM 检查Python（优先F盘完整版本）
echo 🐍 检查Python安装（优先完整版本）...
...
for /f "tokens=*" %%i in ('where python') do (
    ...
    if "%%i" == "F:\AI\DigitalHuman_Portable\python.exe" (
        echo   ✅ F盘完整版本Python（推荐）
        goto :python_ok
    )
)

echo ⚠️  建议优先使用F盘的完整版本Python
echo   可以通过环境变量PATH调整优先级
...
if not "%VS_PATH%"=="" (
    ...
    if exist "%VS_PATH%\VC\Tools\MSVC" (
        ...
        echo ❌ C++编译器工具未安装
        echo 请在VS Installer中安装"C++桌面开发"工作负载
    )
    
    if exist "%VS_PATH%\Common7\IDE\CommonExtensions\Microsoft\CMake" (
        echo ✅ VS内置CMake工具已安装
    ) else (
        echo ⚠️  VS内置CMake工具未安装，将尝试使用独立CMake
    )
)
...
if "%VS_PATH%"=="" (
    ...
    if exist "%TEMP_DIR%\vs_buildtools.exe" (
        ...
        REM 重新检测VS路径
        for %%d in (C D E F G) do (
            if exist "%%d:\Program Files\Microsoft Visual Studio\2022\BuildTools\VC\Tools\MSVC" (
                set "VS_PATH=%%d:\Program Files\Microsoft Visual Studio\2022\BuildTools"
                set "VS_YEAR=2022"
                goto :vs_installed
            )
        )
...
REM 如果没有CMake，尝试安装
if "%CMAKE_PATH%"=="" (
    echo ❌ 未找到CMake，建议在VS Installer中安装"C++ CMake工具"
    echo 或者自动下载独立CMake...
    ...
    if exist "%TEMP_DIR%\cmake.msi" (
        ...
        echo ❌ 无法下载CMake
        echo 建议在VS Installer中安装"C++ CMake工具"组件
    )
)
...
echo 
echo # 检查是否使用F盘Python
echo if 'F:\\AI\\DigitalHuman_Portable' in sys.executable:
echo     print^("✅ 使用F盘完整版本Python^(推荐^)"^)
echo else:
echo     print^("⚠️  建议使用F盘完整版本Python"^)
echo print^(^)
echo 
echo # 测试关键库
echo libraries = [
...
echo print^("🔧 您的环境: F盘Python + D盘VS2022"^)
...
if 'F:\\AI\\DigitalHuman_Portable' in sys.executable:
    print('✅ 使用F盘完整版本Python')
else:
    print('⚠️  当前Python路径:', sys.executable)
...
print()
print('🏆 针对您的环境优化完成！')
print('💡 F盘Python + D盘VS2022 + VS内置CMake')
" 2>nul
...
echo 4. 在VS Installer中安装"C++ CMake工具"组件
echo.
echo 📞 获取帮助:
echo 1. 查看上方的错误信息
echo 2. 运行debug_install_enhanced.bat重新诊断
echo 3. 参考README_COMPLETE.md文档
echo.
goto :final_pause
...
echo.
echo 💡 您的环境已针对F盘Python + D盘VS2022优化
echo.
